...
class InstagramUnofficialService
{
    ...
    public function authenticate(string $username, string $password): array
    {
        try {
            ...
            if (isset($respArr['two_factor_info'])) {
                ...
                if (! empty($respArr['two_factor_info']['whatsapp_two_factor_on'])) {
                    $verification_method = '6';
                }
                if (! empty($respArr['two_factor_info']['totp_two_factor_on'])) {
    ...
    public function postPhoto(string $filePathOrUrl, string $caption = '', $options = []): array
    {
        ...
        try {
            if (! file_exists($filePath) || ! is_readable($filePath)) {
                if ($isTemp && $filePathRelative) {
                    $this->deleteTempFile($filePathRelative);
                }
            ...
            if (! empty($options['pin']) && $mediaId) {
                $this->pinPost($mediaId);
            }
            if (! empty($options['first_comment']) && $mediaId) {
                $this->commentOnMedia($options['first_comment'], $mediaId);
            }
    ...
    public function createVideoThumbnail($videoPath, $outputJpgPath, $second = 1)
    {
        ...
        do {
            ...
            if ($response->successful()) {
                ...
                if ($response->cookies()) {
                    ...
                    if ($midCookie && $midCookie->getValue()) {
                        $this->authData['mid'] = $midCookie->getValue();
                    } else {
                        $this->authData['mid'] = $mid;
                    }
                    if ($csrfCookie && $csrfCookie->getValue()) {
                        $this->authData['csrftoken'] = $csrfCookie->getValue();
                    }
                } else {
                    $this->authData['mid'] = $mid;
                }
