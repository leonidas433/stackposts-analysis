...
class PlanService
{
    ...
    public function syncPlanPermissions(array $currentPermissions): array
    {
        ...
        foreach (\Module::allEnabled() as $module) {
            ...
            $menu = $module->get('menu');
            if (! isset($menu['role']) || $menu['role'] !== 'client') {
                continue;
            }

            $alreadyExists = collect($currentPermissions)->contains(fn ($perm) => is_array($perm) && ($perm['module_name'] ?? null) === ($menu['name'] ?? null)
            );
            if ($alreadyExists) {
                continue;
            }
    ...
    public static function assignPlanOnRegister($userId, $planIdSecure = null)
    {
        $user = User::find($userId);
        if (! $user) {
            return ['status' => 'user_not_found', 'redirect' => route('register')];
        }
