...
class AIService
{
    ...
    public function __construct(OpenAIService $openAIService, DeepSeekService $deepSeekService, GeminiService $geminiService, ClaudeService $claudeService)
    {
    ...
    {
        $maxLength = (int) ($options['max_tokens'] ?? get_option('ai_max_output_lenght', 1000));
        if ($maxLength <= 0) {
            $maxLength = 1000;
        }
